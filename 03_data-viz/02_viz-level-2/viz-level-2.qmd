...
sw.wrangled <- read_csv("sw-wrangled.csv")
...
1. The wrangled dataset excludes some characters
2. The wrangled dataset adds height in cm
3. Characters without heigh in inches in the original are excluded from the wrangled dataset
...
Most of these variables are characters with mass, height_cm, and  height_inches as numeric. To recreate the starwars, plot gender should be a factor. Currently the levels (m and f) are not ordered in the dataset.
...
Gender, mass, and height are mapped onto the plot. The presence of the regression line, grid lines, background color, fonts, and the key presentation.
...
I need to save Female, Male, and other as factors before piping. I also need to assign N/A variables in the gender column to the Other scatterplot.

2. What geoms and aesthetics are used in this plot? Does layer order matter, and if so, how?

<!-- answer below -->

Factor is one aesthetic used in the plot. Geompoint and geomsmooth are also used.
...
I will also need to use a facet layer.
...
This is a faceted scatter plot that presents character height (x) by mass (y) with regression lines for each plot.

table(sw.wrangled$gender)
in

sw.wrangled <- sw_wrangled
...
ggplot(sw.wrangled, aes(x = height_cm, y = mass, color = gender)) +  # Replace x and y with your variable names
  geom_point() +  # Use points for the scatter plot
  geom_smooth(method = "lm") +
  facet_wrap(~ gender, ncol = 3) +
  scale_color_manual(values = c(f = "red", m = "gray", na = "yellow")) +
  labs(title = "Height and weight across gender presentation", subtitle = "A cautionary tale in misleading free axis scales & bad design choices",
  ...
  theme(
    ...
    legend.text = element_text(family = "serif", size = 12),  # Change font to serif, size 12
    legend.title = element_text(family = "serif", size = 14)) +
scale_x_continuous(
    breaks = seq(60, 270, 30),  # Common breaks for all facets
    limits = c(60, 270))
...
I encountered several issues. I could not figure the theme/ color of the original plot. I tried to manually insert colors and it did not look very similar. I also attempted to use the ggsci function scale_color_futurama and the background did not change as expected. I was able to change the color of the lines but I am not sure how this did not work out in the end. I also could not change the color of the box around the title of each plot. I was able to create a key with all three variable but amongst the coding catastrophe this was also lost.
...
I attempted to use ggsci, facet_grid_sc (to change the y scale), sw.wrangled$gender[is.na(sw.wrangled$gender)] <- "Other" to try to assign the na values in the dataset for plotting. I am sure there are more but I lost count.
