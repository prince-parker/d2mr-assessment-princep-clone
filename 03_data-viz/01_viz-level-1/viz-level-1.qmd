...
mpg, cyl, disp, hp, drat, wt, qsec, wt, gear, carb
...
Yes they are. The non-continuous variables are vs (engine) and am (transmission).
...
mtcars.viz[, c(1, 2, 3, 4, 5, 6, 7, 10, 11)] <- lapply(mtcars.viz[, c(1, 2, 3, 4, 5, 6, 7, 10, 11)], as.numeric)

mtcars.viz <- mtcars.viz %>% rename("Engine" = vs, "Transmission" = am)
...
ggplot(mtcars, aes(x = factor(cyl, levels = c(8, 6, 4)))) + 
  ...
  labs(title = "Bar Plot of Cylinder Counts", x = "fct_infreq(n_cylinders)", y = "Count")
...
ggplot(mtcars.viz, aes(x = wt, y = mpg)) +
  geom_point(shape = 17) +
  labs(title = "Scatterplot of Weight by MPG ", x = "Weight", y = "miles_per_gallon")
...
I need to create a factor that I can refer to in the ggplot code to identify 8, 6, and 4 and the x axis factors.

What geoms and aesthetics are used in this plot? Does layer order matter, and if so, how?

<!-- answer below -->

This plot needs the ordinary x and y aesthetics and a cylinder factor that will outline levels based on a previously created factor. I dont believe layer order matters.
...
1. scale_fill_manual to assign colors to each box
2. labs for axis titles
3. scale_x_discrete to order the assigned factors from left to right
...
# Create a factor variable for cyl with the desired order (important for limits)
mtcars.viz$cyl_factor <- factor(mtcars.viz$cyl, levels = c(8, 6, 4))

# Create the boxplot
ggplot(mtcars.viz, aes(x = cyl_factor, y = mpg, fill = cyl_factor)) +
  geom_boxplot() +
  scale_x_discrete(limits = c("8", "4", "6")) +
...
I need to set a limit for low and high value mpg for each part of the faceted scatterplot.

What geoms and aesthetics are used in this plot? Does layer order matter, and if so, how?

<!-- answer below -->

I know I will need to use smooth for the regression lines, and a facet wrap. Of course a labs layer as well.
...
data$mpg_category <- ifelse(data$mpg > median(data$mpg), "High", "Low")
data$mpg_category <- factor(data$mpg_category, levels = c("High", "Low"))

ggplot(data, aes(x = wt, y = disp)) +
  geom_point(color = "purple") + 
    geom_smooth(method = "lm", linetype = "dashed") +
  facet_wrap(~ mpg_category) +
  labs(title = "Weight vs. Displacement by MPG Category",
       subtitle = "A very important analysis",
       x = "Weight",
       y = "Displacement")
...
I need to create some factor for both frequency and class. I also need to specify weight class qualifications

What geoms and aesthetics are used in this plot? Does layer order matter, and if so, how?

<!-- answer below -->

I will need to use color and add the legend.
...
data$weight_class <- cut(data$wt, 
                        breaks = c(0, 2, 3, 4, Inf),  # Example: custom breaks
                        labels = c("Class 1", "Class 2", "Class 3", "Class 4"),
                        include.lowest = TRUE)

# Convert am to factor with labels
data$am <- factor(data$am, levels = c(0, 1), labels = c("Automatic", "Manual"))

# Order weight classes by frequency
class_order <- data %>%
...
data$weight_class <- factor(data$weight_class, levels = class_order)

# Create the stacked barplot
ggplot(data, aes(x = weight_class, fill = am)) +
  geom_bar(position = "stack") +
  labs(title = "Distribution of Transmission Types by Weight Class",
           x = "Weight Class (Ordered by Frequency)",
           y = "Frequency",
...
I was able to recreate all of the plots, but had some challenges with creating weight classes in the last stacked bar plot and with making the colors match in the boxplot and bar graphs.
