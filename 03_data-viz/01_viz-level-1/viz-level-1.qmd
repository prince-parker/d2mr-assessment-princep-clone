...
I know I will need to use smooth for the regression lines.
...
data$mpg_category <- factor(data$mpg_category, levels = c("Low", "High"))
...
# Create weight classes using quantiles (or your preferred method)
data$weight_class <- cut(data$wt,
                         breaks = quantile(data$wt, probs = c(0, 0.25, 0.5, 0.75, 1)), # Example: quantiles
...
# Order the weight classes by total frequency (most frequent to least frequent)
class_order <- data %>%
...
ggplot(data, aes(x = weight_class, fill = am)) +  # x = weight class, fill = transmission
  geom_bar(position = "stack") +  # Stacked bars
  labs(title = "Distribution of transmission types by weight class",
       x = "Weight class ordered by total frequency",
       y = "Frequency",
